...
try:
    ...
    pytest.skip(f"rocquantum_bind module not found. Have you built it? Run 'pip install .' in the project root. Error: {e}", allow_module_level=True)
...
def test_bell_state_simulation():
    """
    Tests the full simulation pipeline by creating a Bell state |Φ+⟩.
    This test relies on the dummy implementation in rocquantum.h.
    """
    num_qubits = 2
    ...
    # The dummy implementation doesn't actually process these gates,
    # but we call them to ensure the API is correctly bound.
    ...
    # The dummy C++ implementation is hardcoded to return the Bell state
    # when num_qubits is 2.
    ...
    assert final_state_vector.dtype == np.complex128, f"State vector should have dtype complex128, but has {final_state_vector.dtype}."
    assert final_state_vector.shape == (2**num_qubits,), f"State vector shape should be {(2**num_qubits,)}, but is {final_state_vector.shape}."
...
def test_custom_gate_application():
    ...
    try:
        ...
        # The dummy C++ code doesn't actually apply the gate.
